...
- [x] T094 [US1] 高程特征的**帧证据数值化**断言（`层=视觉`/统计，**对齐修订后 SC-002**）【**已完成，入口 Agent 两臂各自独立复跑通过**（webgpu `1 passed (5.4s)`、webgl2 `1 passed (4.4s)`，各 exit 0）。三条 MUST 断言实测：**(a) 覆盖率** 画布纹理回读 153600/153600=1.000、合成器 144144/144144=1.000（阈值 0.5）；**(b) 几何↔数据集自洽并列落盘**——数据集 `[-6883, 5775] m`（页面 fetch 全部 **286/286** 片 .hgt 解码）、帧证据 `[-38.4614, 4166] m`（顶点缓冲**第 3 通道** `position3DAndHeight`，78376 顶点 / 8 draw / `nonFiniteFloats 0`）、**第二条独立路径** ECEF 反算（Bowring）`[-38.462, 4166] m`，两路互差 **mean 0.00013 m / max 0.000985 m**；逐顶点残差 median 8 m / p99 70.5 m（阈值 50/250 m）；跨度 4204 m ≥ 3164.5 m；**(c) 非空白**以"每像素都被写过"为数值定义，颜色计数只用 `uniqueColoursGpuReadback`(=1) 并**显式声明不与** `uniqueColoursComposite`(=256) 比较。**反例自检两式均真实变红**：相机移出数据集 → 覆盖率 1.2% < 50%，exit 1；读错顶点通道（lane 0 代 lane 3）→ `-15575.6 m below dataset floor`，exit 1；均已还原并复跑绿。**如实标注的未取得独立证据**：**webgl2 臂无独立高程帧证据**（顶点回读与画布深度接口都是 WebGPU 路径仪器），spec 对该缺失**显式断言"必须报告缺失+原因"**而非静默通过；**深度存在性不断言**——`depth24plus-stencil8` 深度面本机不可拷（G-7 §0），且深度标记层实测**不具区分力**（always/greater/less 三种比较都涂满 153600/153600，原因已定位到 `depthIndicatorInputs` 自带 renderState 把 `depthTest.enabled` 置 false，`RenderState.ts:527-529` 将其映射为 `depthCompare:"always"`），故只断言"该仪器不具区分力"并落盘 `discriminating: false` + note。**口径披露**：逐顶点自洽用"顶点邻域 5×5 内最近数据集样本"（median 8 m），并列发布 rounded-texel 82 m 与注册相位证据（91% 顶点落在采样中心之间）；**未**把相位问题当产品缺陷断言——若要把口径收紧到"逐 texel 复现"，需先由上游确认 mesh 顶点与 `HeightmapTerrainData` 采样中心的相位关系（独立问题，需新证据）。**勘误（入口 Agent 自纠）**：我先前的"T094 收尾快照"提交（`d99ee3f`）误打在其实例 #2 的**中间态**上（含 `HEIGHT_LANE = 0` 与一处日志取值 bug），已由本次重新快照更正。】原始要求：（a）**覆盖率**：非背景色像素占比 ≥ 阈值（阈值落盘并记录来源）；（b）**几何↔数据集自洽**：从帧证据（顶点/深度/矩阵）读出的高程量级与固定数据集一致（区间与来源一并落盘）；（c）不出现空白/纯背景色/整片单色（`uniqueColorCount`；仪器名 MUST 用 `uniqueColoursGpuReadback`，**不得**与 `uniqueColoursComposite` 混用，两者口径不同）。**起伏/明暗调制显式后置**：近地相机下上游 `fade = clamp((far − near)/(…), 0, 1) = 0` ⇒ `finalColor = color × lightColor`（实测，非推测；见 plan 的起伏决策与 A1/A2 证据），两条路径行为一致，故本任务 **MUST NOT** 断言高度-亮度相关性、阴影或遮挡差异。`自检`：`node tests/support/backend-runner.mjs --backend=webgpu --suite=visual:terrain-elevation` 与 `--backend=webgl2 --suite=visual:terrain-elevation` 串行各以 0 退出。〖二选一〗`→ SC-002（rev）, FR-001`
...
- [ ] T163（**追加编号**；由 T092 上报，**产品代码缺陷，优先级高**）**uniform 环形缓冲容量装不下多瓦片帧 ⇒ 整帧渲染失败 + 近空白帧；且近地细化到数据集不存在的层 ⇒ `tilesLoaded` 永不收敛**。实测（T092，两例相机：5.2 km / pitch −6 与 24 km / pitch −20）：`DEFAULT_COMMAND_SLOTS=8`（`backend-webgpu/webgpu/uniform-writer.ts:395`）在 `uniform ring buffer is full: 9216 bytes hold 9 slot(s)` 处失败 → **1498 条 render error**、合成器帧基本空白（前景仅 **1461 px**，即 credit 覆盖层）；另一条：近地细化到 **level 13**（固定数据集只有 **0–12** 层）⇒ `tilesLoaded` 在 30 s 内**永不为真**（1500 帧空转）。要求：(a) 环形缓冲 MUST 按帧内命令数扩容或提供**明确的降级策略**（例如"超容量即按可渲染子集绘制并给出可诊断提示"），**MUST NOT** 静默丢命令或整帧空白；(b) 细化请求超出数据集层数时 MUST 停在该数据集的**最大层**而不是无限请求不存在的层，并使 `tilesLoaded` 能收敛（或如实返回 `{loaded:false}` 并给出原因）；(c) 两条各配单元/契约断言与反例自检；(d) 修好后 MUST 按 `reference-frames/**/*.tolerance.json` 的 `baselineState.regenerationPolicy` **重新生成两条路径的参考帧**（相机可渲染范围会随之扩大，参考帧条件变了）。`层=单元` + `层=契约` + `层=视觉`。`→ FR-030/FR-015, SC-001/SC-002(rev), T092 实测` —— **触发条件已收窄为"机位相关"（入口 Agent 用观察页实测，2026-09-19）**：在固定 960×640 画布上扫描机位，得到

| 机位 | draws | tiles | 结果 |
|---|---|---|---|
| 24 km / **pitch −50**（陡俯视） | 12 | 28 | **正常**（0 错误） |
| 24 km / pitch −5（地平线入画） | 0 | 8 | `uniform ring buffer is full` |
| 12 km / pitch −20 | 0 | 8 | 同上 |
| 8 km / pitch −10 | 0 | 8 | 同上 |
| 6 km / pitch −8 | 0 | 17 | 同上 |

⇒ **只要地平线进入视野就必然溢出，而且瓦片更少、命令反而更多**（说明多出来的命令不来自瓦片数）；一旦溢出，该帧失败并**永久毒化**后续帧（紧随其后就是 T152 的逐帧 `error-scope … scopes still open`，`drawCallCount` 归零、`whenTilesLoaded` 不再收敛）。**因此当前管线只能渲染"陡俯视"机位**——T092 视觉套件把相机定在 `24 km / pitch −50` 不是审美选择，而是被本缺陷逼出来的唯一可渲染机位。修复要求：(a) 环形缓冲 MUST 按**帧内实际命令数**扩容（或在容量之外给出明确降级），使地平线入画的常规机位可用；(b) 溢出 MUST NOT 永久毒化后续帧（与 T152 一并验证：溢出后下一帧应能自行恢复）；(c) 修好后 MUST 重新生成两路参考帧并**扩大套件覆盖的机位集合**（至少加入一个地平线入画的机位，否则该缺陷会再次逃过验收）。**可复现入口**：`node tools/scripts/serve.mjs --port 8451` 后打开 `http://127.0.0.1:8451/artifacts/demo-viewer/?w=960&h=640&height=24000&pitch=-5`（该观察页由入口 Agent 建立，非交付物）。
