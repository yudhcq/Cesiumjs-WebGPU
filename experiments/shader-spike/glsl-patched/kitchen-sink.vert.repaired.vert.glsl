...
layout(std140, set = 0, binding = 0) uniform CZM_UBO {
    ...
    vec2 u_verticalExaggerationAndRelativeHeight;
    vec3 u_center3D;
...
layout(location = 0) in vec4 compressed0;
layout(location = 1) in float compressed1;
...
layout(location = 2) in vec3 geodeticSurfaceNormal;
...
layout(location = 3) out vec3 v_positionMC;
layout(location = 4) out vec3 v_positionEC;

layout(location = 5) out vec3 v_textureCoordinates;
layout(location = 6) out vec3 v_normalMC;
layout(location = 7) out vec3 v_normalEC;
...
layout(location = 8) out float v_distance;



layout(location = 9) out vec3 v_atmosphereRayleighColor;
layout(location = 10) out vec3 v_atmosphereMieColor;
layout(location = 11) out float v_atmosphereOpacity;
...
void main()
{
    ...
    height = height * (czmUBO.u_minMaxHeight.y - czmUBO.u_minMaxHeight.x) + czmUBO.u_minMaxHeight.x;
    position = (czmUBO.u_scaleAndBias * vec4(position, 1.0)).xyz;
    ...
    float exaggeration = czmUBO.u_verticalExaggerationAndRelativeHeight.x;
    float relativeHeight = czmUBO.u_verticalExaggerationAndRelativeHeight.y;
    float newHeight = (height - relativeHeight) * exaggeration + relativeHeight;


    float minRadius = min(min(czmUBO.czm_ellipsoidRadii.x, czmUBO.czm_ellipsoidRadii.y), czmUBO.czm_ellipsoidRadii.z);
    ...
    v_normalMC = normalMC;
    v_normalEC = czmUBO.czm_normal3D * v_normalMC;
    ...
    v_distance = length( (czmUBO.czm_modelView3D * vec4(position3DWC, 1.0)).xyz);
