// ============================================================================================
// G-5 gate — WGSL `czm_` prelude for the terrain closure (tasks.md T022–T024).
//
// Every function below is a **faithful port** of the upstream GLSL builtin it names (source of
// truth: `node_modules/@cesium/engine/Source/Shaders/Builtin/**`, ported 1:1 including constant
// values). The ports are mechanical except for the constructions WGSL does not have, each of
// which is called out in place:
//   W1 GLSL function overloading does not exist in WGSL      → distinct names (`_2` / `_3` / `_4` suffix).
//   W2 `atan(y, x)` is `atan2(y, x)` in WGSL.
//   W3 GLSL struct constants (`czm_emptyRaySegment`) do not exist → a `valid` flag on the struct.
//   W4 `texture(s, uv)` uses implicit LOD (needs uniform control flow in WGSL) → `textureSampleLevel(..., 0.0)`.
//   W5 `gl_FrontFacing == false` → `!input.front_facing` (`@builtin(front_facing)`).
//   W6 `float(bool)` → `select(0.0, 1.0, b)`.
//   W7 GLSL `bool` uniforms are stored as `u32` (WGSL uniform address space has no bool — G-4/H-4).
//
// This file is consumed by `wgsl-emitter.mjs`, which runs the self-built preprocessor
// (`glsl-preprocess.mjs`) over it with the variant's define set and prepends the generated
// uniform struct, bindings and varying structs.
// ============================================================================================
