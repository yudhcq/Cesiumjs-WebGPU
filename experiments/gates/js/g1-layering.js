 ...
 */
import { CesiumWidget, Color, Credit, Event, GeographicTilingScheme, HeightmapTerrainData, Rectangle, TerrainProvider, TileAvailability, TileProviderError, } from "cesium";
/* -------------------------------------------------------------------------- */
...
const frameTimesMs = [];
let fixtureHeights = new Map();
let fixtureVerified = null;
const G1TerrainProvider = function () {
...
G1TerrainProvider.prototype = Object.create(TerrainProvider.prototype);
G1TerrainProvider.prototype.constructor = G1TerrainProvider;
G1TerrainProvider.prototype.getLevelMaximumGeometricError = function (level) {
    const radius = this.tilingScheme.ellipsoid.maximumRadius;
...
G1TerrainProvider.prototype.getTileDataAvailable = function (x, y, level) {
    return findTile(level, x, y) !== undefined;
};
G1TerrainProvider.prototype.loadTileDataAvailability = function () {
    return Promise.resolve();
};
G1TerrainProvider.prototype.requestTileGeometry = function (x, y, level) {
    ...
    if (!heights) {
        if (findTile(level, x, y)) {
            ...
            this.errorEvent.raiseEvent(new TileProviderError(this, `G-1 local heightmap fixture for tile ${key} is not loaded`, new Error("fixture-missing")));
        }
        return undefined;
    }
...
async function loadFixture() {
    ...
    for (const tile of TILES) {
        ...
        const ok = bytes.byteLength === FIXTURE_BYTES && digest === tile.fnv1a32 && meta.fnv1a32Hex === toHex(tile.fnv1a32);
        allOk = allOk && ok;
        tiles[key] = {
 ...
 */
function probeContextType(canvas) {
...
`;
async function initWebgpuLayer(canvas) {
    const layer = {
        ok: false,
        reason: null,
        adapterInfo: null,
    ...
    new Float32Array(vertices.getMappedRange()).set([-0.96, -0.96, -0.62, -0.96, -0.96, -0.62]);
    vertices.unmap();
    layer.ok = true;
...
let webgpuVertices = null;
let markerEnabled = false;
function renderWebgpuLayer(layer) {
...
function colorStats(snap, rgb, tolerance, minAlpha) {
    const { data } = snap;
    let matched = 0;
    let opaque = 0;
    let transparent = 0;
...
function diffSnapshots(a, b, perChannelTolerance = 8) {
...
/* -------------------------------------------------------------------------- */
const state = {
    route: "control-opaque",
    canvasAContext: null,
    canvasBContext: null,
    webgpu: null,
    htmlCanvasContext: null,
    skyEnabled: false,
    ready: false,
    fatal: null,
};
...
async function bootstrap() {
    ...
    applyRoute(requestedRoute && ROUTE_NAMES.includes(requestedRoute)
        ? requestedRoute
        : "control-opaque");
    ...
    if (state.webgpu.ok) {
        console.log(`[g1] WebGPU adapter: ${JSON.stringify({
    ...
    scene.globe.tileLoadProgressEvent.addEventListener((remaining) => {
...
function snapshotCanvasA(tag) {
    return new Promise((resolve, reject) => {
        ...
        remove = widget.scene.postRender.addEventListener(() => {
            ...
            try {
                snapshots.set(tag, readCanvas(canvasA));
                resolve();
            }
            catch (error) {
                reject(error);
...
const g1 = {
    ...
    layerInfo() {
        ...
        const canvasesInDocumentOrder = Array.from(container.querySelectorAll("canvas")).map((element) => element.className);
