...
//
// GLSL source of truth: node_modules/@cesium/engine/Source/Shaders/GlobeVS.js
// Ported from the verified leaf (webgpu/wgsl/leaves/terrain-vs.wgsl);
...
fn vs_main(input: VSIn) -> VSOut {
  ...
  var out: VSOut;
  // tasks.md T074: the GL clip-space z produced by `u_modifiedModelViewProjection` is remapped to
  // the WebGPU NDC range here — the **only** `@builtin(position)` write of this stage, and the other
  // half of the pair lives in the prelude (`czms_unprojectDepth`). `gl_FragDepth` does not exist in
  // the terrain closure, and WGSL's `@builtin(frag_depth)` is already [0,1] on both APIs.
